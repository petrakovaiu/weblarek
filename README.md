...
3 - содержит поля, используемые внутри класса:
`private items: IProduct[]`; - хранит массив товаров в магазине.
`private item: IProduct | null`; - хранит товар для подробного отображения.
4 - содержит методы:
...
2 - содержит поля, используемые внутри класса:
`private itemsInCart: IProduct[] = [];`- поле для хранения товаров.
2 - методы:
...
2 - методы:
`setData(newData: IBuyer): void` - сохранение данных в модели. Один общий метод или отдельные методы для каждого поля. Важно учесть, что должна быть реализована возможность сохранить только одно значение, например, только адрес или только телефон, не удалив при этом значения других полей, которые уже могут храниться в классе;
`getData(): IBuyer` - получение всех данных покупателя;
`clearData(): void` - очистка данных покупателя;
`validateField(field: keyof IBuyer): boolean` - валидация данных, принимает поле
`validateAll(): { [key in keyof IBuyer]: boolean }` - валидация всех данных


###### Слой коммуникации

Класс слоя коммуникации для взаимодействия классов и API прилоджения. Экземпляр коммуникотивного слоя принимает объект, соответствующий интерфейсу IApi , что нужно для разделения логики.
...
Методы класса:
    `getProduct(): Promise<ApiResponse>` - Нужен для get запроса на эндпоинт /product/ и возвращает объект, полученный от сервера, в котором находится массив товаров
    ...
    `getSelectedProduct(uri: string): Promise<IProduct>` - метод принимает строку, а возвращает выбранный продукт
    `postProduct(uri: string, byerData: PostData): Promise<IProduct>` - Принимает строку, данные покупателя и данные о покупке.