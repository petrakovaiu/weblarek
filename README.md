...
`_items: IProduct[]`; - хранит массив товаров в магазине.
`_item: IProduct | null`; - хранит товар для подробного отображения(по умолчанию пустой).
4 - содержит методы:
...
`_itemsInCart: IProduct[] = [];` - поле для хранения товаров.
2 - методы:
...
покупатель:
`export class Buyer {}`
1 - внутри поля инициализирован объект cо следующими данными:
`_data: BuyerData = {`
